function [h,ax,BigAx,patches,pax] = plotmatrix_v3(varargin)
...
%   $Revision: 1.19.4.7 $  $Date: 2005/10/28 15:54:39 $
...
for i=1:cols
[f_IS(i,:),xi_IS(i,:)] = ksdensity(x(:,i));
[f(i,:),xi(i,:)] = ksdensity(z(:,i));
end
...
%     patches(i,:) = bar(histax,xx,nn,'hist');
    ...
    plot(xi(i,:),f(i,:),'k',xi_IS(i,:),f_IS(i,:),'r');%plot(xi_IS(i,:),f_IS(i,:),'k',xi_IS_1,f_IS_1,'r');
