...
function l = loglikelihood(thetas, data)

for i = 1:length(thetas)

  theta = thetas(i);

  if ((((((((theta.K<0|theta.P<=0)|theta.M0<0)|theta.M0>=2*pi)|theta.s<0)|theta.e<0)|theta.e>=1)|theta.omega<0)|theta.omega>=2*pi)
  % This is meant to yield a very low log likelihood when the parameters are out of the acceptable range.
    l(i) = -10^100;
  else
    l(i) = (-length(data.t)/2)*log(2*pi) - (1/2)*sum(log(data.errors.^2+theta.s^2)) - (1/2)*sum(((data.V-model(theta, data.t)).^2)./(data.errors.^2+theta.s^2));
  end
