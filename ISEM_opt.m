function [X,Proposal,Opt] =ISEM_opt(X,Proposal,Proposal0,df,Target_function,gama,j) % One iteration of EM with IS
...
%     aviobj = addframe(aviobj, getframe);
    ...
    X.Target=feval(Target_function,X.Values);%Target_pdf_rastrigin(X.Values);%feval(Target_function,X.Values,data);
    [Opt_tmp,ind]=max(X.Target);
    if Opt_tmp>Opt.value
       Opt.value=Opt_tmp;
       Opt.X=X.Values(ind,:);
    end
